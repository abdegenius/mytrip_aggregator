...
let header = `headers: {
        ...
        'User-Agent': 'Mytrip Ng',
    }`;
const ABC_STATES = require("./../../helpers/AbcStates");
module.exports = fp(async function (fastify, opts) {
  fastify.decorate("ABCCheckTrips", async function (payload) {
    try {
      ...
      if (
        destination_state_terminals.length > 0 &&
        departure_state_terminals.length > 0
      ) {
        let DEPARTURE_STATE,
          DESTINATION_STATE = [];
        const CITIES = await fastify.GDSGetCities();
        if (CITIES.length > 0) {
        ...
        if (DEPARTURE_STATE.length > 0 && DESTINATION_STATE.length > 0) {
          let ROUTES = [];
          const CITY_PAIRS = await fastify.GDSGetCityPairs();
          if (CITIES_PAIRS.length > 0) {
            DEPARTURE_STATE.forEach((depState) => {
              DESTINATION_STATE.forEach((desState) => {
                CITY_PAIRS.map((city) => {
                  if (
                    city[0] == depState.origin_id &&
                    city[1] == desState.destination_id
                  ) {
          ...
          if (ROUTES.length > 0) {
            const promises = ROUTES.map(async (route) => {
              const response = await axios({
                ...
                url:
                  api +
                  `availabilities/${route.departure_state_id}/${route.destination_state_id}/${payload.trip_date}.json`,
                headers: {
                  api_key: key,
                  "Content-Type": "application/json",
                  'User-Agent': 'Mytrip Ng',
            ...
            if (TRIPS.length > 0) {
              ...
              TRIP_DATA.forEach(async (trip) => {
                ...
                let boarding_data =
                  schedule_data.data.result.bus_layout.boarding_stages
                    .trim()
                    .split("|");
                let dropping_data =
                  schedule_data.data.result.bus_layout.dropoff_stages
                    .trim()
                    .split("|");
                let service = schedule_data.data.result.service_name
                  .toUpperCase()
                  .includes("ABC");
                if (service) {
                  DATA.push({
                    provider: {
                      name: "ABC Transport",
                      short_name: "ABC",
                      logo: "https://access.mytrip.ng/providers/abc.jpg",
                    },
                    ...
                    total_seats:
                      schedule_data.data.result.bus_layout.total_seats,
        ...
        return {
          error: true,
          message:
            "FAILED: Check provided states for errors, states does not exist",
  ...
  fastify.decorate("ABCBookTrip", async function (payload) {
    ...
    try {
      const GET_BOOKING = await axios.post(
        `${api}tentative_booking/${trip_id}.json?api_key=` + key,
        {
          book_ticket: {
            seat_details: {
              seat_detail:
                Number(seats.length) <= 1 ? seat_detail[0] : seat_detail,
            },
            contact_detail: {
      ...
      if (GET_BOOKING.data.result) {
        ...
        const COMPLETE_BOOKING = await axios.post(
          `${api}confirm_booking/${result.pnr_number}.json?api_key=` + key,
          {
            headers: {
              api_key: key,
              "Content-Type": "application/json",
            ...
            data: [
              {
                ...
                amount_per_seat:
                  parseFloat(response.total_fare) /
                  parseFloat(response.no_of_seats),
                order_number: response.operator_pnr,
                vehicle_no: response.bus_type,
                narration:
                  response.origin.toUpperCase() +
                  " TO " +
                  response.destination.toUpperCase(),
